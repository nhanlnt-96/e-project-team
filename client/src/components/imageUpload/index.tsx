...
const ImageUpload: React.FC<IProps> = ({ imageUrl, onRemoveImage, ...props }) => {
  ...
  const handleConvertFileToUri = (file: File) => {
    fileToDataUri(file).then((uri) => setImageUri(uri as string));
  };
  ...
  const handleUploadImage = (event: ChangeEvent<HTMLInputElement>) => {
    if (props.onChange) {
      ...
      handleConvertFileToUri(event.currentTarget.files?.[0] as File);
    }
  };
    ...
    <div className='w-full space-y-4'>
      ...
      {imageUri || imageUrl ? (
        <div className='relative w-full'>
          {!imageUrl || imageUri ? (
            <ButtonComp className='!px-2 !py-2 absolute right-0 z-10 hover:bg-antd-status-error' onClick={handleRemoveImage}>
              <svg
                fill='none'
                stroke='currentColor'
                strokeWidth='1.5'
